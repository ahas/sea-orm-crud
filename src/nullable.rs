...
use sea_orm::{
    ActiveValue, ColIdx, ColumnType, IntoActiveValue, QueryResult, Set, TryGetError, TryGetable,
    Value,
};
        ...
        D: Deserializer<'de>,
    {
            ...
            T: Deserialize<'de>,
        {
            ...
            where
                E: de::Error,
            {
                // JSON에서 필드가 없는 경우 Undefined로 간주
    ...
    T: Into<Value> + sea_orm::sea_query::Nullable,
{
    fn into_active_value(self) -> ActiveValue<Option<T>> {
        match self {
            Nullable::Value(v) => Set(Some(v)), // 값이 있는 경우 Some(v)로 감쌈
            Nullable::Null => Set(None),        // null일 경우 None으로 설정
            Nullable::Undefined => NotSet,      // undefined일 경우 NotSet
        }
    }
}
